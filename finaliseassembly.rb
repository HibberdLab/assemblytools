...
File.open("#{indir}renamed.#{infile}", "w") do |output|
  i = 0
  File.open("#{indir}centroids.#{infile}", "r").each do |line|
    if line =~ /^>/
      line = ">transcript_#{i}"
      i += 1
    end
...
"clustering with similarity cutoff #{opts.idthreshold}"
